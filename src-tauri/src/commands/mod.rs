...
fn build_enhanced_path() -> String {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        // standalone 安装目录（汉化版独立安装包）
        extra.push(format!("{}/.openclaw-bin", home.display()));
        extra.push("/opt/openclaw".into());
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        // 6.5 standalone 安装目录（汉化版独立安装包，自带 Node.js 运行时）
        ...
        if !localappdata.is_empty() {
            extra.push(format!(r"{}\Programs\OpenClaw", localappdata));
            extra.push(format!(r"{}\OpenClaw", localappdata));
        }
        extra.push(format!(r"{}\OpenClaw", pf));
